    ...
    if (orderDetails) {
      ...
      for (let i = 0; i < updatedStatuses.length; i++) {
        ...
        if (
          statusKey === 'received' ||
          (statusKey === 'preparing' &&
            ['preparing', 'out_for_delivery', 'delivered'].includes(orderDetails.status)) ||
          (statusKey === 'out_for_delivery' &&
            ['out_for_delivery', 'delivered'].includes(orderDetails.status)) ||
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2">
            <div>
              <p className="text-gray-600">
                Mã đơn: <span className="font-semibold">{orderDetails?.orderId}</span>
              </p>
              <p className="text-gray-600">
                Đặt lúc: <span className="font-semibold">{orderDetails?.timeOrdered}</span>
              </p>
            </div>
            <div>
              <p className="text-gray-600">
              ...
              {orderStatuses.map((status, index) => (
                <div
                  ...
                  className={`flex flex-col items-center p-3 rounded-lg ${
                    status.completed ? 'text-green-700 bg-green-50' : 'text-gray-400 bg-gray-50'
                  }`}
                >
                  <div
                  ...
                  {status.timestamp && <span className="text-sm mt-1">{status.timestamp}</span>}
                </div>
              ))}
            </div>
          </CardContent>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="p-4 bg-blue-50 rounded-md">
                  <p className="text-sm text-blue-700">
                    <strong>Vấn đề giao hàng?</strong> Liên hệ đội hỗ trợ để được trợ giúp ngay với
                    đơn hàng của bạn.
                  </p>
                </div>
              </CardContent>
